...
class circles_Yard {
  constructor(x, y, dx, dy, radius, color, mass) {
    ...
    this.update = () => {
      if (
        this.x + this.radius > w + this.radius ||
        this.x - this.radius < 0 - this.radius
      ) {
        this.velocity.x = -this.velocity.x;
      }
      if (
        this.y + this.radius > h + this.radius ||
      ...
      for (var i = 0; i < circles_arraY.length; i++) {
        ...
        if (
          distance(this.x, this.y, circles_arraY[i].x, circles_arraY[i].y) -
            this.radius * 2 <
          0
        ) {
...
for (let i = 0; i < 20; i++) {
  ...
  if (i !== 0) {
    ...
    for (var j = 0; j < circles_arraY.length; j++) {
      ...
      if (
        distance(x, y, circles_arraY[j].x, circles_arraY[j].y) - radius * 2 <
        0
      ) {
